...
- 1 x       or       stack - C9300/X/L (-M versions), C9200/L (-M versions), or MS150/MS130
...
When a cloud enabled downstream       is connected to the MX secure router and is initially booted, all       ports will, by default, be part of VLAN 1.  The       will initiate DHCP requests for an IP address on all connected interfaces.  The       will receive an IPv4 address in the 192.168.128.2 - 254 address range from the MX secure router.  Likewise, when a downstream cloud managed wireless AP is subsequently connected to the       and is initially booted, the AP will initiate DHCP requests for an IP address on its uplink port.  The AP will receive an IPv4 address in the 192.168.128.2 - 254 range from the MX secure router.  The MX secure router firewall is, by default, configured to allow all outbound traffic to the Internet initiated internally on VLAN 1, and to NAT the traffic to the IP address of the WAN interface.  This allows the downstream       and APs to initially connect to the Cisco Cloud.
...
- The port connecting the MX secure router to the downstream       must be configured as a trunk port with the native VLAN being the default of VLAN 1.
- The ports connecting the       to downstream cloud managed APs must be configured as trunk ports with the native VLAN being the default of VLAN 1. 
...
As of the 19.1.x firmware release, MX secure routers do not support link aggregation (otherwise known as EtherChannel) of LAN ports for connectivity between an MX secure router and a       or       stack.  MX secure routers also do not support the concept of a logical Port Channel interface or spanning-tree protocol (STP).

The MX67 and MX68 platforms only support 1 Gbps Ethernet RJ45 connections between the MX secure router and       or       stack. The MX85 supports 1 Gbps Ethernet RJ45 or 1 Gbps SFP (optical) connections.  However, the bandwidth per port is still limited to 1 Gbps.  The MX95 and MX105 platforms support 1 Gbps Ethernet RJ45 connections as well, but also support 10 Gbps SFP+ (optical) connections. 

When using an MX platform that only supports 1 Gbps RJ45 or SFP connections, a single connection, configured as a trunk port supporting all VLANs configured within the branch, provides only 1 Gbps of bandwidth and no redundancy between the MX secure router and the       or       stack.  Depending upon the WAN transport speeds and the requirements for inter-VLAN routing within the branch, this may or may not be sufficient.  For additional bandwidth, the following options may be possible.

- A single pair of redundant 1 Gbps Ethernet RJ45 or SFP connections configured as trunk ports supporting all VLANs.  STP on the       should put one of the connections into blocking mode, ensuring there are no loops between the       or       stack and the MX secure router.  This potential option addresses redundancy, but not additional bandwidth between the MX secure router and the       or       stack.

- Multiple pairs of redundant 1 Gbps Ethernet RJ45 or SFP (optical) connections configured as trunk ports, each supporting a subset of the overall VLANs.  STP on the       will put one of the connections of each pair into blocking mode, ensuring there are no loops between the       and the MX secure router.  This potential option addresses both redundancy and aggregate bandwidth across all VLANs (not per individual VLAN) between the MX secure router and the       or       stack.  Note that this has not been validated or included as part of Unified Branch - Branch as Code for this release.

The use of SFP (optical) connectivity may also be needed in situations where the MX secure router and the       are physically separated from each other.  Distances greater than 100 meters, or facilities where only fiber optic cabling exists between wiring closets may result in the use of the SFP ports.  

When using an MX platform which supports 10 Gbps SFP+ (optical) connections, you must ensure the       model of the       or       stack you choose to deploy within the small branch also supports 10 Gbps SFP+ (optical) connections.

### Small Branch Switch / Switch Stack ###
For this release of Cisco Unified Branch - Branch as Code, a single       or       stack consisting of C9300/X/L (-M versions), C9200/L (-M versions), or MS150/MS130      es provides LAN connectivity to devices within the small branch.  All      es within a single stack should be the same model.

Support for PoE on       ports may be needed when connecting wireless LAN (WLAN) APs, IP phones, surveillance cameras, and other devices to      es.  You must select       models that support the necessary power per port for the devices you plan on using within the branch, as well as the overall power budget based on the number of such devices. The total power budget for PoE devices depends on the number of power supplies installed within the      , as well as the power rating of the individual power supplies. 

When C9300/X/L or C9200/L Series      es are managed by the Cisco Cloud (formerly known as the Meraki Cloud), the       ports (or port_ids as they are referred to in the YAML configuration files within this repository) on the       begin with port/port_id 1, and proceed up to the last port/port_id of the      , including the uplinks which are typically the highest port/port_id numbers (25-29, 49-52, etc.).  This is unlike       configurations in traditional IOS-XE platforms, where the port numbering reflects the position of the       in the       stack, the module within the      , and the port position on the      .  For instance, the first       uplink port of a standalone IOS-XE       would be referred to as port GigabitEthernet 1/1/1.

The link speed and duplex of the Ethernet ports on the MX secure router and the      es must match for the uplink to come up active.  A best practice is to leave the link speed for auto-negotiation of speed and duplex.

802.1x authentication, MAC Authentication Bypass (MAB), or Meraki Secure Port are not currently supported on the uplink between the MX secure router and the       models supported for Unified Branch - Branch as Code.  Hence, a best practice is for the equipment to be in a secure area within the site, such as a wiring closet in which only authorized personnel are allowed to enter.

For Unified Branch - Branch as Code, uplink       ports are configured as a trunk ports allowing VLANs 1, 10, 20, 30, 50, and 999 on the MX secure router with VLAN 1 being the native VLAN.  This configuration allows      es to automatically acquire an IPv4 address in the 192.168.128.0/24 subnet from the MX secure router and connect to the Cisco (formerly Meraki) Cloud when initially deployed.  Upon configuration, the management subnet of the       is moved from VLAN 1 to VLAN 999.  This allows monitoring flows such as SNMP, Syslog, and NetFlow to be initiated from the      es and sent over the Auto VPN tunnels to centralized servers.

Note that the       may initially need to be manually configured via console connection to operate as a Cisco Cloud managed       before being connected to the MX secure router.
...
All WLAN AP models listed in the table above support only a single uplink port for connectivity to the      .  As mentioned previously, the default VLAN (VLAN 1) automatically configured on MX is used to initially ensure downstream unconfigured      es and APs can connect to the Cisco Cloud to onboard.  All VLANs with customer traffic (Data, Voice, IoT, Infra, and Guest) are configured as additional VLANs.  This means that the connections between      es and APs must be configured as trunk ports.  The native VLAN of the trunk ports should be configured for VLAN 1.

#### PoE Requirements for the Access Points ####
When configuring the uplink between the       and APs, the network administrator needs to consider how power is to be supplied to the APs.  Power can be supplied via one of the following: 

- A       which supports PoE, and a       port that supplies the necessary power (in terms of Watts) required for the model of AP.

- An inline power injector placed within the wiring closed next to the       in cases where there is insufficient power budget left within the      , or in cases where the       does not support PoE or the correct power levels required by the AP. 
...
For Cisco Unified Branch - Branch as Code, the method which has been selected for providing power to the APs is via the       ports.  Therefore, you must select the model of       such that there are sufficient       ports, each with sufficient power (30W or 45W) to support all of the APs within your branch deployment. 
...
If AP uplink speeds greater than 1 Gbps are required, then the network administrator must also consider whether the APs should be connected to       ports which support Multi-Gigabit Ethernet (mGig) speeds of 2.5, 5, or 10 Gbps.  This depends upon the model of AP deployed.  Therefore, as with PoE requirements discussed above, you must select the model of       and AP such that both the       port and the AP supports port speeds greater than 1 Gbps if necessary. 
 
As with the uplinks between the MX secure router and the      , link speed and duplex (full or half) of the Ethernet ports on the       and the AP must match for the uplink to come up active.  Again, a best practice is to typically leave the link speed for auto-negotiation of speed and duplex. 

#### Authentication of the AP to the Switch ####

For this release of Unified Branch - Branch as Code, no authentication of the AP to the       port was configured. The       models supported for Unified Branch - Branch as Code do not support Meraki Secure Port.  Likewise, the AP models do not support the 802.1x supplicant necessary for 802.1x authentication of the AP to the      . Hence, the only alternatives are either no authentication or MAC Authentication Bypass (MAB).  MAB is not ideal because the MAC address of the AP is normally printed on the AP, making it somewhat easy to circumvent.  However, given the physical installations of the APs are typically 8-12 feet off the ground in the ceilings of sites, it is still preferable to no authentication, and would generally be recommended.  Note that use of MAB has not been validated between the AP and the       for this release.
...
LAN connectivity is implemented by a single       or       stack within the small branch. LAN connectivity consists of wired LAN ports for connectivity as uplinks to the MX secure router (trunk ports), uplinks to the access points (also trunk ports), and all wired LAN clients.  

The *access_     _01_name* and *access_     _02_name* sections of the *small_branch_inventory* template within the *data/templates-inventory.nac.yaml* file provides examples of the configuration of       ports for small branch.  Add or delete these sections based on the number of      es in the       stack for the small branch to be deployed. Note that you may need to adjust the port ranges for *access* and *trunk* ports to reflect your needs as well as the number of ports on the model of       which you deploy.  Then, within the *     * template within the *templates-     .nac.yaml* file locate the *     _stacks* section.  Adjust the variables for the names of the      es to match those configured in the *small_branch_inventory* template.

#### LAN QoS (Ingress Classification & Marking) ####

LAN QoS (ingress classification & marking) consists of DSCP to CoS queue mapping configuration as well as rules which classify and set the DSCP marking of traffic.  The *     * template within the *data/templates-     .nac.yaml* file provides an example DSCP to CoS queue mapping assuming a 12 traffic-class QoS model, using all 8 CoS queues, as well as an example set of QoS classification & marking rules for different VLANs on the      .  Both are applied at the       level, meaning the DSCP to CoS queue mapping and the QoS classification & marking rules apply to all ports of the       to which the template is applied.
...
Access control policies consists of hybrid authentication which supports both 802.1x and MAB for       ports to which the policy is applied.  This is configured within the *access_policies* section of the *     * template in the *templates-     .nac.yaml* file.  External RADIUS and RADIUS accounting servers are specified via variables.

#### Power over Ethernet (PoE) ####

 PoE is enabled by default on       ports which support PoE.  Therefore, the port level *PoE* setting (Boolean - true or false) does not appear within the *small_branch_inventory* template within the *data/templates-inventory.nac.yaml* file.  A general best practice is to leave PoE enabled on       ports and allow the       port to negotiate whether power is to be supplied to the device connected to the       port. 
...
The storm control configuration itself is hardcoded within the *     * template within the *data/templates-     .nac.yaml* file to restrict broadcast, multicast, and unknown unicast traffic to 30% of the available bandwidth of the       ports.  Tune these parameters if necessary.  Determining the appropriate levels for "normal" broadcast, multicast, and unknown unicast traffic may require monitoring the network first for a period, and then setting these parameters slightly above those levels, to protect the network.

A general best practice is to leave RSTP protocol enabled on all       ports within a small branch to minimize the chance of an outage or degradation of service resulting from an inadvertent loop in the network - unless there is a situation where you are absolutely sure this is not needed or will cause problems on given       ports.  Given the small branch design consists of only a single       or       stack, the chance of a loop occurring may be low.  However, the benefits from enabling RSTP generally outweigh the minor time delay necessary for a       port to move from a blocking to a forwarding state when first enabled.
...
VLAN trunking is supported on the links between the MX secure router and the       or       stack. On the secure router, this is configured within the *app_ports* template within the *data/templates-appliance.nac.yaml* file.  On      es, this is configured within the *small_branch_inventory* template within the *data/templates-inventory.nac.yaml* file.

VLAN trunking is also supported on the links between the      es and the access points. On the      , this is again configured within the *small_branch_inventory* template within the *data/templates-inventory.nac.yaml* file.  On the access points, Data and Guest SSIDs are mapped to separate VLANs. This is configured via the SSID level *default_vlan_id* setting within the *wireless* template of the *data/templates-wireless.nac.yaml* file. Note that for the Data SSID, the VLAN assignment can be overridden via the Filter-ID attribute corresponding to a Group Policy returned during 802.1x/MAB authentication.

Access ports on the       are hardcoded with a VLAN and voice VLAN configuration within each       definition within the *small_branch_inventory* template within the *data/templates-inventory.nac.yaml* file.  Again, note that you may need to adjust the port ranges for trunks and access ports to match the ports on the       models you deploy within the small branch.
...
> :information_source:
> The upstream       port to which the access point is connected will need to be configured to trust DSCP markings or implement an ingress classification & marking policy if QoS from the wireless client is desired to be maintained. 
...
IP address assignments apply to wired and wireless devices within the small branch.  Such devices can be network infrastructure devices such as the MX secure router,      es, and access points; or end-user devices such as laptops, IP phones, smart phones, tablets, etc.  Within the small branch design, the MX secure router can simultaneously function as a DHCP client, DHCP server, and/or DHCP relay agent.  
...
- Infra VLAN (VLAN 999) - This DHCP pool is used for the management interfaces of       and AP devices once they receive their configuration changing the management VLAN from the default VLAN 1 to the Infra VLAN (VLAN 999).  The IPv4 subnet for the Infra VLAN (VLAN 999) is specified as a variable within the *app_vlans* template within the *templates-appliance.nac.yaml* file. In addition, since management IP addresses of network infrastructure should ideally not change - especially when monitoring flows such as NetFlow, SNMP, and Syslog are being generated by these devices - fixed IP address assignments should be configured for these devices, as is shown in the *app_vlans* template.
...
Because VLAN trunking is enabled between the MX secure router and the Layer 2       or       stack, all wired end-user devices which connect to the       are expected to received IP address either from the DHCP server pools within the MX secure router, or be relayed across the Auto VPN tunnel to centralized DHCP servers for the respective VLAN to which the device is connected.  

Likewise, because VLAN trunking is enabled between the Layer 2       or       stack and the access points, and because the SSID definitions found within the *wireless* template within the *data/templates-wireless.nac.yaml* specify *bridged mode* for IP address assignment, wireless clients are also expected to receive IP addresses from the DHCP server pools within the MX secure router, or be relayed across the Auto VPN tunnel to centralized DHCP servers for the respective SSID (which is mapped to a specific VLAN) to which the device is connected.

#### SNMP Services ####

SNMP polling can be used to query and gather information either from the Cisco (formerly Meraki) dashboard or directly from devices (secure routers,      es, and access points) within networks / sites.
...
- 802.1x authentication of devices connected to wired       access ports or the wireless Data SSID, with fallback to MAB for devices without 802.1x supplicant support, such as printers.
